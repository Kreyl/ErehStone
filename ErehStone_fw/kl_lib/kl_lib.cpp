...
void Timer_t::SetupPrescaler(uint32_t PrescaledFreqHz) {
    uint32_t Freq = Clk.GetTmrClkFreq(ITmr);
...
uint32_t Clk_t::GetTmrClkFreq(TIM_TypeDef* PTim) {
#if defined STM32L1XX
    if(ANY_OF_3(PTim, TIM9, TIM10, TIM11)) {
        uint32_t APB2prs = (RCC->CFGR & RCC_CFGR_PPRE2) >> 8;
        uint32_t TimerClkMulti = (APB2prs < 4)? 1 : 2;
        return APB2FreqHz * TimerClkMulti;
    }
    else {
        uint32_t APB1prs = (RCC->CFGR & RCC_CFGR_PPRE1) >> 8;
        uint32_t TimerClkMulti = (APB1prs < 4)? 1 : 2;
        return APB1FreqHz * TimerClkMulti;
    }
#else
#error "GetTmrClkFreq: bad MCU"
#endif
}
